...
def get_user(db: Session, username: str):
    return db.query(model.User).filter(User.username == username).first()


def get_user_by_email(db: Session, email: str):
    return db.query(model.User).filter(User.email == email).first()
...
def create_user(db: Session, user: UserCreate):
    ...
    db_user = model.User(username=user.username, email=user.email, hashed_password=fake_hashed_password)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
